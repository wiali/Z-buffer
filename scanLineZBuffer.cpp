...
inline static ClassifiedBorder *caculateBorder(RealVertex *top, RealVertex *low,
                                               ClassifiedPolygon *source) {
  ClassifiedBorder *p =
      static_cast<ClassifiedBorder *>(malloc(sizeof(ClassifiedBorder)));
  p->x = top->x;
...
void ScanLineZBuffer::ConstructClassifiedTable(const std::vector<Mesh> &meshes,
  ...
  for (auto iter = meshes.begin(); iter != meshes.end(); ++iter) {
    ...
    for (unsigned int i = 0; i != vertices.size(); ++i) {
      ...
      //realVertices[i].x = standardScale * (temp.x + 0.5f);
      //realVertices[i].y = standardScale * (temp.y + 0.5f);
      //realVertices[i].z = standardScale * (temp.z + 0.5f);
      realVertices[i].x = (temp.x/temp.w);
      realVertices[i].y = (temp.y/temp.w);
      realVertices[i].z = (temp.z/temp.w);
    }
    ...
    for (int i = 0; i < sizeOfIndices; i += stepLength) { // 按片元节点数步进
      ...
      if (ymax->y != ymedium->y) {
        ClassifiedBorder *temp =
            caculateBorder(ymax, ymedium, polygon[ymax->y]);
      ...
      if (ymedium->y != ymin->y) {
        ClassifiedBorder *temp =
            caculateBorder(ymedium, ymin, polygon[ymax->y]);
...
void ScanLineZBuffer::ZBuffer() {
  ...
  for (int scanline = height - 1; scanline >= 0; --scanline) {
    clearZBuffer(zbuffer, -1*height*width, width);
    // 从分类多边形表加入活化表
    ...
    while (classifiedPolygon) {
      // 填充DynamicPolygon结构
      DynamicPolygon *p =
          static_cast<DynamicPolygon *>(malloc(sizeof(DynamicPolygon)));
      ...
      // 检测该面是否还有第三条边
      if ( classifiedBorder && (classifiedBorder->source == q->source)) {
